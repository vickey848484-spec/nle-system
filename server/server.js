...
// 静态文件目录：项目根目录（兼容 Render 本地两种部署方式）
...
function writeAll(table, data) {
  fs.writeFileSync(path.join(DATA_DIR, table + '.json'), JSON.stringify(data, null, 2), 'utf8');
}
...
function requireAuth(req, res, next) {
  ...
  if (!token || !tokens.has(token)) return res.status(401).json({ error: '未登录或登录已过期' });
...
for (const table of TABLES) {
  if (table === 'operation_logs') {
    app.get(`/api/${table}`, requireAuth, (req, res) => {
      ...
      res.json(readAll(table).slice(0, limit));
    });
    continue;
  }
